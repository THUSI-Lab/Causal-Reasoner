...
try:
    ...
    from .gemini_physical_logic_score_core import (
        build_gemini_physical_logic_score_messages,
        parse_gemini_physical_logic_score_response,
    )
...
SCORER_NAME = "gemini_physical_logic_score_ranker"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Score QA rows with Gemini physical-logic axes and retain top fraction.")
    ...
    parser.add_argument("--min-overall-score", type=float, default=75.0)
    parser.add_argument("--min-axis-score", type=float, default=50.0)
...
def score_one(sample: Any, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if issues:
        ...
        try:
            ...
            messages = build_gemini_physical_logic_score_messages(sample, frames)
            ...
            score = parse_gemini_physical_logic_score_response(raw_response, task_name=sample.task_name)
...
def _min_axis(score: dict[str, Any]) -> float:
    keys = [
        "precondition_validity_score",
        "causal_dependency_score",
        "state_transition_score",
        "timeline_consistency_score",
        "physical_feasibility_score",
    ]
    return min(float(score.get(key, 0.0)) for key in keys)
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.parallel)) as pool:
        ...
        for idx, future in enumerate(as_completed(futures), start=1):
            ...
            if idx % 25 == 0 or idx == len(futures):
                print(f"[gemini-score] processed {idx}/{len(futures)}", flush=True)

    results.sort(key=lambda x: (-float(x["score"]["overall_physical_logic_score"]), x["task_name"], x["sample_id"]))
    keep_n = _selected_count(len(results), args.top_fraction, args.top_k)
    selection_rule = (
        f"{'top_k=' + str(keep_n) if args.top_k > 0 else 'top_fraction=' + str(args.top_fraction)};"
        f"min_overall_score={args.min_overall_score};min_axis_score={args.min_axis_score}"
    )
    ...
    for rank, item in enumerate(results, start=1):
        score = item["score"]
        rank_selected = rank <= keep_n
        threshold_selected = (
            float(score.get("overall_physical_logic_score", 0.0)) >= args.min_overall_score
            and _min_axis(score) >= args.min_axis_score
        )
        selected = rank_selected and threshold_selected
        if selected:
            selected_count += 1
        score_meta = {
        ...
        row = attach_filter_metadata(item["row"], "gemini_physical_logic_score", score_meta)
    ...
    write_jsonl(args.output_dir / "gemini_physical_logic_scores.jsonl", records)

    bands = Counter()
    for item in results:
        score = float(item["score"]["overall_physical_logic_score"])
    ...
    (args.output_dir / "gemini_physical_logic_score_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
