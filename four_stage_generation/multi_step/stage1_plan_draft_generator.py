...
def _draft_hard_errors(draft: Dict[str, Any]) -> List[str]:
    ...
    if len(steps) < 3:
        errors.append(f"Too few steps: got {len(steps)} (required >= 3). Split broad steps into finer sub-goals.")
    if len(steps) > 9:
    ...
    for i, step in enumerate(steps, start=1):
        ...
        if and_count >= 5 or comma_count >= 5:
            errors.append(
    ...
    *,
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if step_count and not (4 <= step_count <= 7):
            warnings.append(f"Step count is {step_count} (preferred 4-7; hard constraint 3-9).")
