...
def run_stage2_for_video(
    ...
    *,
    ...
    if not isinstance(steps_for_count, list) or not (3 <= len(steps_for_count) <= 9):
        raise RuntimeError(
            f"Draft step count must be within [3, 9] for the four-stage pipeline (got {len(steps_for_count) if isinstance(steps_for_count, list) else 'N/A'}). "
